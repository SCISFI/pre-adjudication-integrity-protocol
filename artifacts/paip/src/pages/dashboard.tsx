...
export default function Dashboard() {
  ...
  const submittedWeeks = new Set(
    (submissions ?? []).map((s: { weekNumber: number }) => s.weekNumber),
  );
  const latestSubmission = (submissions ?? []).slice(-1)[0] as
    | {
        weekNumber: number;
      ...
      <div className="space-y-8">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Pre-Adjudication Integrity Protocol — Phase 1
          </p>
        </div>
          ...
          <section className="space-y-3">
            ...
            <div className="rounded border border-border p-5 bg-card space-y-3">
              <p className="text-sm text-foreground leading-relaxed">
                {latestSubmission.feedback.feedbackText}
              </p>
