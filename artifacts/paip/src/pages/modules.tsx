...
function getUnlockStatus(weekNumber: number, submissions: Submission[]) {
  const currentWeekSubmitted = submissions.some(
    ...
    <Layout>
      <div className="space-y-6">
        ...
        <div className="rounded border border-border bg-card overflow-hidden divide-y divide-border">
          {(modules ?? []).map(
            (mod: { weekNumber: number; title: string; focusArea: string }) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-4 px-5 py-5 text-left transition-colors group ${
                    locked
                      ? "bg-muted/20 cursor-not-allowed opacity-75"
                      : "hover:bg-muted/30"
                  }`}
        ...
        <p className="text-xs text-muted-foreground">
          Testing mode: all Phase 1 modules are temporarily unlocked so the full
          weekly workflow can be reviewed. The 7-day unlock timing can be
          restored later.
        </p>
      </div>
    </Layout>
